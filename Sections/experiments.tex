...
attention-target alignment with \(H_t\) or attention mass on robot regions.
...
optimizer, and the grounding loss weight \(\lambda_g\).
...
state-aligned target distribution, \textsc{ProprioVLA} attention, and trajectory
overlays for the same scene. The comparison checks whether attention remains
dominated by task objects and background context or shifts toward the arm,
gripper, and end-effector while the trajectory remains stable.
...
(2) replacing normalized KL with an unnormalized localization loss; (3) using
raw optical flow instead of state-aligned residual motion; and (4) replacing
the learned grounding queries with state-derived queries. Each ablation is
...
to mine visual self-grounding targets, rather than merely receive
proprioceptive values as extra inputs. The comparison separates three
alternatives: gains from additional state, gains from motion saliency alone,
and gains from learned-query self-grounding.
