...
passive input token, \textsc{ProprioVLA} uses proprioceptive change to derive
visual supervision from state-aligned residual flow and uses this supervision
to train learned grounding queries. This signal highlights residual motion that
is both locally salient in the image and aligned with proprioceptive change,
...
heatmaps and a normalized KL grounding objective. First, \textsc{ProprioVLA}
mines heatmaps from robot interaction by selecting residual motion that is
salient in the image and aligned with proprioceptive change, while downweighting
uncertain frames. These heatmaps supervise a lightweight learned-query adapter
over visual tokens, encouraging the policy representation to localize the
visible robot body without manual masks, external grounding datasets, or
supervision from stronger foundation models.
...
the role of the learned-query grounding path: state-aligned heatmaps provide
spatial supervision, and normalized KL supervision stabilizes attention
learning across target scales. These results suggest that self-supervised
...
\begin{itemize}
    ...
    that trains learned grounding queries with normalized KL supervision to
    improve task success and robustness in real-robot and simulation
    environments.
\end{itemize}
